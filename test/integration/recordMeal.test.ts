...
beforeAll(async () => {
  const url = process.env.MCP_CONFIRMATION_TEST_REDIS_URL;
  ...
  await raw.connect();
  first = await connectConfirmationStore(url);
  cleanup.push(first.close);
  second = await connectConfirmationStore(url);
...
function confirm(
  ...
  return callTool(target, 'record_meal', { ...args, confirmation_token: confirmation }, credential);
}
...
function writeReply(once = false, delayMs?: number) {
  mock.route({
    ...
    method: 'POST',
    status: 201,
    body: { success: true, data: { ...storedMeal, user_id: '7', idempotency_key_hash: 'hidden' } },
    once,
...
describe('record_meal over MCP and shared Redis', () => {
  ...
  it('rejects changed arguments without consuming the original confirmation', async () => {
    ...
    writeReply();
    const confirmation = await proposal();
    expectRejected(await confirm(confirmation, otherServer, { ...meal, calories: 201 }));
    expect(mock.callsTo('/api/meallog/me')).toHaveLength(0);
  ...
  it('rejects an expired or unknown confirmation without a write', async () => {
    ...
    expectRejected(await confirm(confirmation));
    expectRejected(await confirm('z'.repeat(43)));
    expect(mock.callsTo('/api/meallog/me')).toHaveLength(0);
  });
