...
if (configured === undefined)
  console.warn(
    'SKIP confirmation Redis integration: MCP_CONFIRMATION_TEST_REDIS_URL is absent. CI supplies disposable Redis.'
  );

describe.skipIf(configured === undefined)('Redis-backed confirmation gate', () => {
  const keys: string[] = [];
  const cleanup: (() => void)[] = [];
  ...
  function request(confirmationToken: string) {
    return { ...action, confirmationToken, requestDeadlineMs: 10_000 };
  }
  ...
  describe('confirmation store with real Redis and independent connections', () => {
    ...
    it('recovers after a backend commit whose response was lost, using the same key', async () => {
      ...
      await expect(first.execute(request(confirmationToken), backend)).rejects.toMatchObject({
        code: 'confirmation_write_failed',
      });
  ...
  it('reports a live lease before expiry and later returns the completed result', async () => {
    const { confirmationToken } = await issue();
    const gate = deferred<JsonValue>();
    const started = deferred<undefined>();
    const write = vi.fn(() => {
    ...
    try {
      await adjust(confirmationToken, { expires_at: 0 });
      await expect(second.execute(request(confirmationToken), write)).resolves.toMatchObject({
        state: 'in_progress',
      });
    } finally {
      gate.resolve(record);
  ...
  it('does not retain private writer fields in Redis or its replay', async () => {
    const { confirmationToken } = await issue();
    const write = vi
      .fn()
      .mockResolvedValue({ ...record, meal: action.arguments, private: 'secret'.repeat(20_000) });
    await first.execute(request(confirmationToken), write);
    expect(JSON.parse(String((await inspect(confirmationToken)).result))).toEqual(record);
    await expect(second.execute(request(confirmationToken), write)).resolves.toMatchObject({
      result: record,
    });
    expect(write).toHaveBeenCalledTimes(1);
  ...
  it('recovers the same store after a dropped socket and refuses work during reconnection', async () => {
    ...
    const proxy = createServer((incoming) => {
      ...
      incoming.on('error', () => {
        /* The store reports the connection failure. */
      });
      if (!available) {
        incoming.destroy();
        return;
      }
      const outgoing = connect({ host: target.hostname, port: Number(target.port || 6379) });
    ...
    const isolated = await connectKeyValue(proxied.href, 500);
    const store = createConfirmationStore(isolated);
    const write = vi.fn().mockResolvedValue(record);
    try {
      const { confirmationToken } = await issue(store);
      ...
      await vi.waitFor(async () => {
        await expect(isolated.eval('return 1', [], [], 100)).rejects.toThrow();
      });
      await expect(store.execute(request(confirmationToken), write)).rejects.toMatchObject({
        code: 'confirmation_store_unavailable',
      });
      ...
      await vi.waitFor(
        async () => {
          expect(await isolated.eval('return 1', [], [], 100)).toBe(1);
        },
        { timeout: 4_000 }
      );
      await expect(store.execute(request(confirmationToken), write)).resolves.toMatchObject({
        state: 'done',
        replayed: false,
      });
      expect(write).toHaveBeenCalledTimes(1);
  ...
  // Fault injection executes real Lua in real Redis. The assertions below must fail for
  // each independently removed guard; canned eval replies cannot satisfy this gate.
  it.each(['binding comparison', 'done short-circuit'] as const)(
    'detects removal of the %s from the actual claim script',
    async (guard) => {
      const removed =
        guard === 'binding comparison'
      ...
      expect(CLAIM_CONFIRMATION).toContain(removed);
      const mutant = CLAIM_CONFIRMATION.replace(removed, '');
      const mutatedStore = createConfirmationStore({
        eval: (script, ...args) =>
          connection.eval(script === CLAIM_CONFIRMATION ? mutant : script, ...args),
      });
      const { confirmationToken } = await issue();
      const write = vi.fn().mockResolvedValue(record);
      async function safetyAssertion() {
        if (guard === 'binding comparison') {
          await expect(
            mutatedStore.execute(
              { ...request(confirmationToken), arguments: { ...action.arguments, calories: 999 } },
              write
            )
          ...
          await expect(
            mutatedStore.execute(request(confirmationToken), write)
          ).resolves.toMatchObject({ state: 'done', replayed: false });
          await expect(
            mutatedStore.execute(request(confirmationToken), write)
          ).resolves.toMatchObject({ state: 'done', replayed: true });
          expect(write).toHaveBeenCalledTimes(1);
        }
      }
      await expect(safetyAssertion()).rejects.toMatchObject({ name: 'AssertionError' });
    }
  );
});
