 ...
 *
 * Asserts the wire response, never an error class. When reading a failure: a rejection that
 * never happens is `src/auth/tokenValidator.ts`, and a rejection that happens without a Bearer
 * challenge, or after the live introspection call, is the transport wiring.
 */
